...
function preload() {
  /*
  * TODO:
  - Add background noise for the whole time
  - Change the orientation of the text to a portrait level design.
  - Add loading screen!
  */
...
function imageLoaded(ev) {
  ...
  game.world.resize(1400, 2000);
  game.world.setBounds(0, 0, 1400, g_levelPixelMap[g_levelPixelMap.length-1].y);
  loadSentencesVert();
}
...
function loadSentencesVert() {
  ...
  for (i = 0; i < sentences.length; i++) {
    ...
    for (j = 0; j < words.length; j++) {
      ...
      if ((blockUsed >= level.blockSize) || (word.width() + 10 + blockUsed > level.blockSize)) {
        blockUsed = 0;
        if (g_levelPixelMap[blockCount].y === g_levelPixelMap[blockCount + 1].y) {
          wordX += word.width() + 10;
          // wordY = g_levelPixelMap[blockCount].y;
        }
        else {
          wordY = g_levelPixelMap[blockCount].y;
          wordX = g_levelPixelMap[blockCount].x;
